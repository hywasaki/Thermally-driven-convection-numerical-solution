...
program main_explicit11_12
  ...
  implicit none
  !---------------- parametros ------------------------------------------
  integer,parameter :: nx=2000, nt=2000000, nt_stabilizer=5000000, nq = 15
  real(dp),parameter :: dx=0.5_dp, dt=4.0e-4_dp
  real(dp),parameter :: Le=20._dp, Sc=400._dp, Ra=-2._dp, q_max=0.3_dp
  ...
  real(dp):: growth_rate_new = 0, growth_rate_old = 0
  logical :: flag_growth_rate = .true.
  !!---------------- auxiliares ------------------------------------------
  integer :: i,j,k,save_cnt,prog_step, jj, growth_steps = 1000
  real(dp) :: t0,tcur,pct
      ...
      !======================================================================
        ...
        if ( i > 100000 .and. mod(i, growth_steps) == 0 .and. flag_growth_rate) then
          ...
          if (Cq_max_old == 0._dp) then        
              ...
              if (abs(growth_rate_old - growth_rate_new) < 1.0e-10) then
