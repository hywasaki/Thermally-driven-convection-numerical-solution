...
program main_explicit11_12
  ...
  implicit none
  !---------------- parametros ------------------------------------------
  integer,parameter :: nx=2000, nt=2000000, nt_stabilizer=5000000
...
! Concentration (Con)
  ...
  if (normalized_sc) then
   ...
   b_vort(2:nx-1)= -2./dx**2-q**2-1/(Sc*dt)
  ...
  do i=0,nt
    ...
    ! Vorticity (Vort)
    if (normalized_sc) then
      d_vort(2:nx-1)= -Wq_old(2:nx-1)/(sc*dt) - q*Ra*Tq_old(2:nx-1)
    else
    ...
    !======================================================================
    ! Boundary conditions
    a_con(2)  = 0.0_dp; d_con(1) = 0.0_dp ! Cq(1) = 0
    c_con(nx-1) = 0.0_dp; d_con(nx)= 0.0_dp ! Cq(nx) = 0 
    ! Since C = C0 + perturbation Cq, BC state C(1) = 0, since C0(1) = 1 then Cq(1) = 1
